...
test.describe('API Schema Compatibility Tests', () => {
  ...
  test.describe('Property Endpoints', () => {
    ...
    test('POST /properties - Valid Request', async ({ request }) => {
      ...
      // Should return 201 or 401 (if not authenticated)
  ...
  test.describe('File Upload Endpoints', () => {
    ...
    test('POST /documents/upload - Valid File Upload', async ({ request }) => {
      // Create a test file
      ...
      if (response.status() === 201) {
        const data = await response.json();
        expect(data).toHaveProperty('id');
        expect(data).toHaveProperty('title', 'Test Document');
      }
    });

    test('POST /properties/{id}/media - Valid Image Upload', async ({ request }) => {
      // Create a test image file
      const testImage = new File(['fake image content'], 'test.jpg', { type: 'image/jpeg' });
      
      const response = await request.post(`${API_BASE_URL}/properties/1/media`, {
        multipart: {
          files: [testImage]
        }
      });
      
      // Should return 201 or 401/404 (if not authenticated or property doesn't exist)
  ...
  test.describe('Schema Field Validation', () => {
    ...
    test('Property - Required Fields Validation', async ({ request }) => {
      ...
      // Check that specific field errors are mentioned
