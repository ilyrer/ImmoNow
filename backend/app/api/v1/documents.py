...
async def search_documents(
    ...
    documents, total = await documents_service.search_documents(
        search_request=search_request, offset=offset, limit=pagination.size
    )

    return PaginatedResponse.create(
        items=documents, total=total, page=pagination.page, size=pagination.size
    )
...
@router.get(
    "/{document_id}/versions/{version_id}", response_model=DocumentVersionResponse
)
...
async def toggle_favorite(
    ...
    documents_service = DocumentsService(tenant_id)
    is_favorite = await documents_service.toggle_favorite(
...
async def update_document(
    ...
    document = await documents_service.update_document(
        document_id, current_user.user_id, update_data
    )
...
async def get_document_activity(
    ...
    documents_service = DocumentsService(tenant_id)
    activities = await documents_service.get_document_activities(document_id)
...
async def get_document_comments(
    ...
    documents_service = DocumentsService(tenant_id)
    comments = await documents_service.get_document_comments(document_id)
...
async def add_document_comment(
    ...
    documents_service = DocumentsService(tenant_id)
    comment = await documents_service.add_document_comment(
        document_id, current_user.user_id, comment_data.get("text", "")
    )
...
async def log_document_view(
    ...
    """Log that a document was viewed"""

    documents_service = DocumentsService(tenant_id)
    await documents_service.log_document_view(document_id, current_user.user_id)


@router.post("/{document_id}/download", status_code=status.HTTP_204_NO_CONTENT)
async def log_document_download(
    document_id: str,
    current_user: TokenData = Depends(require_read_scope),
    tenant_id: str = Depends(get_tenant_id),
):
    """Log that a document was downloaded"""

    documents_service = DocumentsService(tenant_id)
    await documents_service.log_document_download(document_id, current_user.user_id)


@router.post("/{document_id}/share", status_code=status.HTTP_204_NO_CONTENT)
async def log_document_share(
    document_id: str,
    current_user: TokenData = Depends(require_read_scope),
    tenant_id: str = Depends(get_tenant_id),
):
    """Log that a document was shared"""

    documents_service = DocumentsService(tenant_id)
    await documents_service._log_activity(
        document_id, current_user.user_id, "shared", {}
    )
