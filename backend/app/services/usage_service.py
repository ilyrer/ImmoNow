...
from app.db.models import (
    UserProfile, Property, Document, PropertyImage, 
    PropertyDocument, BillingAccount
)
...
class UsageService:
    ...
    async def get_usage_snapshot(tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            property_images_count = await sync_to_async(PropertyImage.objects.filter(
                property__tenant_id=tenant_id
            ).count)()
            
            # Property Documents zählen
            property_documents_count = await sync_to_async(PropertyDocument.objects.filter(
                property__tenant_id=tenant_id
            ).count)()
    ...
    async def _calculate_storage_usage(tenant_id: str) -> int:
        ...
        try:
            ...
            property_images_size = await sync_to_async(
                PropertyImage.objects.filter(property__tenant_id=tenant_id)
                .aggregate(total=models.Sum('size'))['total'] or 0
            )()
            
            # Property Documents Storage
            property_documents_size = await sync_to_async(
                PropertyDocument.objects.filter(property__tenant_id=tenant_id)
                .aggregate(total=models.Sum('size'))['total'] or 0
            )()
            
            # Summe in MB
            total_bytes = documents_size + property_images_size + property_documents_size
        ...
        try:
            ...
            return {
                ...
                'properties': {
                    ...
                    'available': calculate_available(usage['properties'], limits['properties']),
                    'percentage': calculate_percentage(usage['properties'], limits['properties']),
                    'unlimited': limits['properties'] == -1
                },
                'storage': {
                    'current_gb': usage['storage_gb'],
                    'limit_gb': limits['storage_gb'],
                    'available_gb': calculate_available(usage['storage_gb'], limits['storage_gb']),
                    'percentage': calculate_percentage(usage['storage_gb'], limits['storage_gb']),
            ...
            return {
                ...
                'properties': {
                    ...
                    'percentage': min(100.0, (usage['properties'] / limits['properties']) * 100),
                    'unlimited': False
                },
                'storage': {
                    'current_gb': usage['storage_gb'],
                    ...
                    'percentage': min(100.0, (usage['storage_gb'] / limits['storage_gb']) * 100),
                    'unlimited': False
                },
                'features': {
                    'integrations': False,
