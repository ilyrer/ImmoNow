...
from app.schemas.properties import (
    ...
    ContactPersonCreate,
    PropertyFeatures as PropertyFeaturesSchema,
...
class PropertiesService:
    ...
    async def create_property(
        self, property_data: CreatePropertyRequest, created_by_id: str
    ) -> PropertyResponse:
    ...
    async def update_property(
        self, property_id: str, property_data: UpdatePropertyRequest, updated_by_id: str
        ...
        def update_property_sync():
            try:
                property_obj = Property.objects.get(
                    id=property_id, tenant_id=self.tenant_id
                )
            except Property.DoesNotExist:
                return None
            ...
            if "address" in update_data and update_data["address"]:
                ...
                address.postal_code = address_data.get(
                    "postal_code", address_data.get("zip_code", address.postal_code)
                )
            ...
            features, created = PropertyFeatures.objects.get_or_create(
                property=property_obj
            )
    ...
    async def _build_property_response(
        ...
        def build_response_sync():
            ...
            return PropertyResponse(
                ...
                # Location coordinates
                coordinates_lat=(
                    float(property_obj.coordinates_lat)
                    if property_obj.coordinates_lat
                    else None
                ),
                coordinates_lng=(
                    float(property_obj.coordinates_lng)
                    if property_obj.coordinates_lng
                    else None
                ),
                # Additional data
    ...
    async def get_property_metrics(self, property_id: str) -> Dict[str, Any]:
        """
        Get property performance metrics from stored data and portal APIs.
        Fetches live data from connected portals and updates the database.
        """
        ...
        def get_metrics_sync():
            ...
            days_on_market = (
                datetime.now().date() - property_obj.created_at.date()
            ).days
            ...
            # Get historical snapshots for chart data (last 30 days)
            thirty_days_ago = datetime.now().date() - timedelta(days=30)
            snapshots = PropertyMetricsSnapshot.objects.filter(
                property=property_obj, date__gte=thirty_days_ago
            ).order_by("date")
            ...
            for i in range(30):
                ...
                if date in snapshot_dict:
                    ...
                    # No data for this day - estimate based on metrics
                    daily_views = max(1, metrics.total_views // max(1, days_on_market))
                    daily_inquiries = max(
                        0, metrics.total_inquiries // max(1, days_on_market)
                    )
                    chart_data.append(
                        {
                            "date": date.isoformat(),
            ...
            if metrics.total_views > 0:
                conversion_rate = (metrics.total_inquiries / metrics.total_views) * 100
            ...
            return {
                ...
                "averageViewDuration": metrics.avg_view_duration or 120,
                "conversionRate": round(conversion_rate, 2),
                "chartData": chart_data,
                "portalBreakdown": {
                    "immoscout24": {
                        "views": metrics.immoscout_views,
                        "inquiries": metrics.immoscout_inquiries,
                        "favorites": metrics.immoscout_favorites,
                    },
                    "immowelt": {
                        "views": metrics.immowelt_views,
                        "inquiries": metrics.immowelt_inquiries,
                        "favorites": metrics.immowelt_favorites,
                    },
                },
                "lastSyncedAt": (
                    metrics.last_synced_at.isoformat()
    ...
    async def sync_property_metrics_from_portals(
        self, property_id: str
    ) -> Dict[str, Any]:
        """
        Sync property metrics from all connected portals.
        Called by background task or on-demand.
        """
        from app.services.immoscout_service import ImmoScoutService
        from app.services.immowelt_service import ImmoweltService

        @sync_to_async
        def get_property_and_jobs():
            try:
                property_obj = Property.objects.get(
                    id=property_id, tenant_id=self.tenant_id
                )
            ...
            # Get all published jobs for this property
            jobs = list(
                PublishJob.objects.filter(
                    property=property_obj,
                    status="published",
                    portal_property_id__isnull=False,
                ).values("portal", "portal_property_id")
            )

            return property_obj, jobs

        property_obj, jobs = await get_property_and_jobs()

        # Fetch metrics from each portal
        ...
        for job in jobs:
            try:
                if job["portal"] == "immoscout24":
                    immoscout_service = ImmoScoutService()
                    portal_metrics = await immoscout_service.get_property_metrics(
                        job["portal_property_id"], self.tenant_id
                    )
                    immoscout_metrics = {
                        "views": portal_metrics.get("views", 0),
                        "inquiries": portal_metrics.get("inquiries", 0),
                        "favorites": portal_metrics.get("favorites", 0),
                    }
                elif job["portal"] == "immowelt":
                    immowelt_service = ImmoweltService()
                    portal_metrics = await immowelt_service.get_property_metrics(
                        job["portal_property_id"], self.tenant_id
                    )
                    immowelt_metrics = {
                        "views": portal_metrics.get("views", 0),
                ...
                print(f"Error syncing metrics from {job['portal']}: {e}")
                continue
        ...
        def update_metrics():
            ...
            metrics.save()  # This will auto-calculate totals

            # Create today's snapshot
            today = datetime.now().date()
            snapshot, _ = PropertyMetricsSnapshot.objects.update_or_create(
                property_id=property_id,
                date=today,
                defaults={
                    "views": metrics.total_views,
        ...
        updated_metrics = await update_metrics()

        return {
            "success": True,
            "property_id": property_id,
            "total_views": updated_metrics.total_views,
            "total_inquiries": updated_metrics.total_inquiries,
            "synced_at": updated_metrics.last_synced_at.isoformat(),
        }

    async def upload_images(
        self, property_id: str, files: List[UploadedFile], uploaded_by_id: str
        ...
        def upload_images_sync():
            ...
            max_order = (
                property_obj.images.aggregate(models.Max("order"))["order__max"] or 0
            )
