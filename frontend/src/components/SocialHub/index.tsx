 ...
 * Verwendet Live-Daten vom Backend
 */
...
const SocialHubIndex: React.FC = () => {
  ...
  // Fetch live dashboard data from backend - bei Fehler werden Default-Werte verwendet
  const { data: dashboardData, isLoading } = useSocialHubDashboard();

  // Extract stats with defaults - immer 0 wenn keine Daten
  ...
  const getActivityConfig = (type: string) => {
    switch (type) {
      case 'post_published':
        return { icon: CheckCircle, color: 'emerald', view: 'queue' as ViewType };
      case 'post_scheduled':
        return { icon: Clock, color: 'blue', view: 'scheduler' as ViewType };
      case 'account_connected':
        return { icon: UserCog, color: 'purple', view: 'accounts' as ViewType };
      default:
        return { icon: Activity, color: 'gray', view: 'overview' as ViewType };
    }
  };
  ...
  // Loading state - nur beim ersten Laden anzeigen
  ...
  // Kein Error-State mehr - zeige immer das Dashboard mit 0-Werten wenn keine Daten
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-6">
                <div className="flex items-center gap-2 px-4 py-2 bg-white/20 dark:bg-[#1C1C1E]/40 rounded-full border border-white/30 dark:border-white/10 backdrop-blur-sm">
                  ...
                  <span className="text-sm font-semibold text-[#1C1C1E] dark:text-white">{stats.connected_accounts} Konten verbunden</span>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-white/20 dark:bg-[#1C1C1E]/40 rounded-full border border-white/30 dark:border-white/10 backdrop-blur-sm">
                  <Calendar className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                  <span className="text-sm font-semibold text-[#1C1C1E] dark:text-white">{stats.scheduled_posts} Posts geplant</span>
                </div>
                {stats.reach_change !== 0 && (
                  <div className="flex items-center gap-2 px-4 py-2 bg-white/20 dark:bg-[#1C1C1E]/40 rounded-full border border-white/30 dark:border-white/10 backdrop-blur-sm">
                    {stats.reach_change > 0 ? (
                    ...
                    <span className="text-sm font-semibold text-[#1C1C1E] dark:text-white">
                      {stats.reach_change > 0 ? '+' : ''}{stats.reach_change.toFixed(0)}% Reichweite
                    </span>
                  </div>
                )}
              </div>
        ...
        <div className="group relative bg-white/10 dark:bg-[#1C1C1E]/30 backdrop-blur-xl rounded-[24px] border border-white/20 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)] p-7 hover:bg-white/15 dark:hover:bg-[#1C1C1E]/40 hover:shadow-[0_12px_48px_rgba(0,0,0,0.18)] hover:scale-[1.02] transition-all duration-300 ease-out cursor-pointer overflow-hidden">
          ...
          <div className="relative flex flex-col h-full">
            <div className="flex items-start justify-between mb-4">
              ...
              {stats.posts_change !== 0 && (
                <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full ${stats.posts_change > 0
                  ...
                  <span className={`text-xs font-semibold ${stats.posts_change > 0
                      ? 'text-green-600 dark:text-green-400'
                      : 'text-red-600 dark:text-red-400'
                    }`}>
                    {stats.posts_change > 0 ? '+' : ''}{stats.posts_change.toFixed(0)}%
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="flex-1">
              ...
              <p className="text-4xl font-bold text-[#1C1C1E] dark:text-white mb-1">{formatNumber(stats.published_posts)}</p>
        ...
        <div className="group relative bg-white/10 dark:bg-[#1C1C1E]/30 backdrop-blur-xl rounded-[24px] border border-white/20 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)] p-7 hover:bg-white/15 dark:hover:bg-[#1C1C1E]/40 hover:shadow-[0_12px_48px_rgba(0,0,0,0.18)] hover:scale-[1.02] transition-all duration-300 ease-out cursor-pointer overflow-hidden">
          ...
          <div className="relative flex flex-col h-full">
            <div className="flex items-start justify-between mb-4">
              ...
              {stats.engagement_change !== 0 && (
                <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full ${stats.engagement_change > 0
                    ? 'bg-green-500/10 dark:bg-green-400/10'
                    : 'bg-red-500/10 dark:bg-red-400/10'
                  }`}>
                  {stats.engagement_change > 0 ? (
                  ...
                  <span className={`text-xs font-semibold ${stats.engagement_change > 0
                      ? 'text-green-600 dark:text-green-400'
                      : 'text-red-600 dark:text-red-400'
                    }`}>
                    {stats.engagement_change > 0 ? '+' : ''}{stats.engagement_change.toFixed(0)}%
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="flex-1">
              ...
              <p className="text-4xl font-bold text-[#1C1C1E] dark:text-white mb-1">{stats.engagement_rate.toFixed(1)}%</p>
              <p className="text-xs text-[#3A3A3C] dark:text-gray-500 flex items-center gap-1.5">
        ...
        <div className="group relative bg-white/10 dark:bg-[#1C1C1E]/30 backdrop-blur-xl rounded-[24px] border border-white/20 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)] p-7 hover:bg-white/15 dark:hover:bg-[#1C1C1E]/40 hover:shadow-[0_12px_48px_rgba(0,0,0,0.18)] hover:scale-[1.02] transition-all duration-300 ease-out cursor-pointer overflow-hidden">
          ...
          <div className="relative flex flex-col h-full">
            <div className="flex items-start justify-between mb-4">
              ...
              {stats.reach_change !== 0 && (
                <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full ${stats.reach_change > 0
                    ? 'bg-green-500/10 dark:bg-green-400/10'
                    : 'bg-red-500/10 dark:bg-red-400/10'
                  }`}>
                  {stats.reach_change > 0 ? (
                  ...
                  <span className={`text-xs font-semibold ${stats.reach_change > 0
                      ? 'text-green-600 dark:text-green-400'
                      : 'text-red-600 dark:text-red-400'
                    }`}>
                    {stats.reach_change > 0 ? '+' : ''}{stats.reach_change.toFixed(0)}%
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="flex-1">
              ...
              <p className="text-4xl font-bold text-[#1C1C1E] dark:text-white mb-1">{formatNumber(stats.total_reach)}</p>
              <p className="text-xs text-[#3A3A3C] dark:text-gray-500 flex items-center gap-1.5">
        ...
        <div className="group relative bg-white/10 dark:bg-[#1C1C1E]/30 backdrop-blur-xl rounded-[24px] border border-white/20 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)] p-7 hover:bg-white/15 dark:hover:bg-[#1C1C1E]/40 hover:shadow-[0_12px_48px_rgba(0,0,0,0.18)] hover:scale-[1.02] transition-all duration-300 ease-out cursor-pointer overflow-hidden">
          ...
          <div className="relative flex flex-col h-full">
            ...
            <div className="flex-1">
              ...
              <p className="text-4xl font-bold text-[#1C1C1E] dark:text-white mb-1">{formatNumber(stats.scheduled_posts)}</p>
              <p className="text-xs text-[#3A3A3C] dark:text-gray-500 flex items-center gap-1.5">
                <Calendar className="w-3.5 h-3.5" />
                Für diese Woche
              </p>
            ...
            <div className="relative">
              <div className="flex items-start justify-between mb-5">
                ...
                <div className="px-3 py-1.5 bg-green-500/10 dark:bg-green-400/10 rounded-full border border-green-500/20">
                  <span className="text-xs font-semibold text-green-600 dark:text-green-400">{stats.connected_accounts} Verbunden</span>
                </div>
              </div>
            ...
            <div className="relative">
              <div className="flex items-start justify-between mb-5">
                ...
                <div className="px-3 py-1.5 bg-orange-500/10 dark:bg-orange-400/10 rounded-full border border-orange-500/20">
                  <span className="text-xs font-semibold text-orange-600 dark:text-orange-400">{stats.scheduled_posts} Geplant</span>
                </div>
              </div>
            ...
            <div className="relative">
              <div className="flex items-start justify-between mb-5">
                ...
                <div className="px-3 py-1.5 bg-pink-500/10 dark:bg-pink-400/10 rounded-full border border-pink-500/20">
                  <span className="text-xs font-semibold text-pink-600 dark:text-pink-400">{stats.pending_posts} Wartend</span>
                </div>
              </div>
        ...
        <div className="space-y-4">
          {recentActivities.length > 0 ? (
            recentActivities.map((activity, index) => {
              ...
              }[config.color] || 'from-gray-500/20 to-gray-600/10 text-gray-600';

              return (
                <div
                  key={index}
                  onClick={() => setCurrentView(config.view)}
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 dark:text-gray-400">Noch keine Aktivitäten vorhanden</p>
              <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                Verbinden Sie Ihre ersten Social Media Konten, um loszulegen
              </p>
            </div>
          )}
        </div>
