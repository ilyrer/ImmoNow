...
const CIMDashboard: React.FC = () => {
  ...
  // API Hook
  const { data: cimData, isLoading, error } = useCIMOverview({
    limit: 10,
    days_back: 30
  });
  ...
  // Load CIM data function
  const loadCIMData = () => {
    // This would trigger a refetch of the CIM data
    // For now, we'll just log that it was called
    console.log('Loading CIM data...');
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-indigo-900">
      ...
      <div className="min-h-screen relative">
        <div className="px-8 py-6">
                              ...
                              <div className="flex items-start space-x-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center space-x-2 mt-2">
                                    ...
                                    <span className={`px-2 py-1 text-xs rounded-full font-medium backdrop-blur-sm border border-white/20 ${getContactStatusColor(contact.status)}`}>
                                      {contact.status}
                                    </span>
                                    {contact.matching_count > 0 && (
          ...
          {/* Render the selected modules */}
          {activeTab === 'sales' && <SalesModule />}
          {activeTab === 'geographical' && <GeographicalModule />}
          {activeTab === 'kpi' && <KPIModule />}
          {activeTab === 'segmentation' && <SegmentationModule />}
