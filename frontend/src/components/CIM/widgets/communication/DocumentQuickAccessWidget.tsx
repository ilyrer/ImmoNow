      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" asChild>
            <Link to="/documents">Alle anzeigen</Link>
          </Button>
        </div>
      </CardHeader>
      <CardContent>
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
        <TabsList className="grid w-full grid-cols-3 mb-4">
          <TabsTrigger value="recent">
            Aktuell {recentCount > 0 && <Badge variant="outline" className="ml-1">{recentCount}</Badge>}
          </TabsTrigger>
          <TabsTrigger value="urgent">
            Dringend {urgentCount > 0 && <Badge variant="destructive" className="ml-1">{urgentCount}</Badge>}
          </TabsTrigger>
          <TabsTrigger value="templates">
            Vorlagen {templates.length > 0 && <Badge variant="outline" className="ml-1">{templates.length}</Badge>}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="urgent" className="mt-0">
          {urgentDocuments.length === 0 ? (
            ...
            urgentDocuments.map((doc) => (
              <div
                key={doc.id}
                className="p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/50 rounded-lg mb-2"
              >
                <div className="flex items-start space-x-3">
